...
test('replays, normalizes and exports a sequence', async ({ page }) => {
  await expect(page.locator('#output')).toHaveText('1. e4 e5 2. Nf3 Nc6 3. Bb5');
...
test('shows restarted games and special moves', async ({ page }) => {
  for (const [example, text] of [['castling', 'O-O'], ['en-passant', 'exd6'], ['promotion', 'bxa8=Q'], ['restart', 'Qh4#']]) {
...
test('chooses an underpromotion on the board', async ({ page }) => {
  await page.locator('#mnemonic').fill('cmp1 a2a4 h7h5 a4a5 h5h4 a5a6 h4h3 a6b7 h3g2');
  await page.locator('#check').click();
  ...
  await page.mouse.click(bounds.x + bounds.width * .5 / 8, bounds.y + bounds.height * .5 / 8);
